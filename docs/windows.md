...
go get -u github.com/bluzelle/ipfs-kubo
...
|`go get -u github.com/bluzelle/ipfs-kubo`       |Fetch / Update `kubo` source|
...
|`mkdir %GOPATH%\src\github.com\ipfs`<br/>`cd %GOPATH%\src\github.com\ipfs`<br/>`git clone https://github.com/bluzelle/ipfs-kubo.git`       |Fetch / Update `kubo` source|
|`cd %GOPATH%\src\github.com\ipfs\kubo` |Change to `kubo` source directory|
|`make install`                            |Build and install to `%GOPATH%\bin\ipfs.exe`|
|`%GOPATH%\bin\ipfs.exe version --all`     |Test the built binary|
...
CMD:
```bat
git clone https://github.com/bluzelle/ipfs-kubo %GOPATH%/src/github.com/bluzelle/ipfs-kubo
cd %GOPATH%/src/github.com/bluzelle/ipfs-kubo/cmd/ipfs
```

PowerShell:
```powershell
git clone https://github.com/bluzelle/ipfs-kubo $env:GOPATH/src/github.com/bluzelle/ipfs-kubo
cd $env:GOPATH/src/github.com/bluzelle/ipfs-kubo/cmd/ipfs
```
...
CMD:
```bat
go install -ldflags="-X "github.com/bluzelle/ipfs-kubo".CurrentCommit=%SHA%"
%GOPATH%\bin\ipfs.exe version --all
```

PowerShell:
```powershell
go install -ldflags="-X "github.com/bluzelle/ipfs-kubo".CurrentCommit=$env:SHA"
...
If you encounter a bug with `kubo` itself (not related to building) please use the [issue tracker](https://github.com/bluzelle/ipfs-kubo/issues) to report it.
