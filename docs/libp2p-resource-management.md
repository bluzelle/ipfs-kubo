...
For example, Kubo maintainers have found in practice that it's a footgun to have too low of a value for `System.ConnsInbound` and a default minimum is used. (See [core/node/libp2p/rcmgr_defaults.go](https://github.com/bluzelle/ipfs-kubo/blob/master/core/node/libp2p/rcmgr_defaults.go) for specifics.)
